...
for (const gp of data.lesGrandsPrix) {
    idGrandPrix.appendChild(new Option(gp.nom, gp.id));
}

// alimenter la zone de liste des pilotes pour l'attribution du point bonus
for (const element of data.lesPilotes) {
    idPilote.appendChild(new Option(element.id + ' ' + element.nom, element.id));
}
